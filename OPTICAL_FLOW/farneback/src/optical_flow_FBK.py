...
SRC_PATH = os.path.join(GLOBAL_PATH, 'DATA','DATASET1','raw','IR')
MASKS_PATH = os.path.join(GLOBAL_PATH, 'OPTICAL_FLOW', 'segmentation','results','figures','best_10_subjects_segmentation_1')
LABELS_PATH = os.path.join(GLOBAL_PATH, 'DATA', 'DATASET1','processed','results','figures','labels_reversed')
CSV_SEG_PATH = os.path.join(GLOBAL_PATH, 'OPTICAL_FLOW','segmentation','results','data','seg_metrics1.csv')
CSV_RBF_FIELD_PATH = os.path.join(GLOBAL_PATH,'OPTICAL_FLOW','farneback','results','data','RBF_pred_1.csv')
...
if DRAW_ON == 1:
    DRAW_SRC_PATH1 = SRC_PATH
    DST_PATH = os.path.join(GLOBAL_PATH, 'OPTICAL_FLOW','farneback','results','figures','RBF_on_IR_pred_paper')

elif DRAW_ON == 2:
    DRAW_SRC_PATH2 = LABELS_PATH
    DST_PATH = os.path.join(GLOBAL_PATH, 'OPTICAL_FLOW','farneback','results','figures','RBF_on_labels_pred')

else:
    DRAW_SRC_PATH3 = os.path.join(GLOBAL_PATH,'IMAGE_REGISTRATION','results','figures')
    DST_PATH = os.path.join(GLOBAL_PATH, 'OPTICAL_FLOW','farneback','results','figures','RBF_on_superposition_pred')

RBF_MATRICES_PATH = os.path.join(GLOBAL_PATH,'OPTICAL_FLOW','farneback','results','data','RBF_pred_matrices1')

os.makedirs(RBF_MATRICES_PATH, exist_ok=True)
...
fovea_center_dict = {'L_03': (1013, 1006), 'L_06': (968, 1078), 'L_15': (1037, 961), 'L_16': (968, 985), 'L_42': (980, 1011), 'L_48': (932, 934), 'L_63': (1042, 961), 'L_78': (1016, 937), 'S_36': (968, 1006), 'S_46': (853, 1120)}
...
def create_subject_data(fname: str) -> dict:
    ...
    if DRAW_ON == 3:
        fname_colored_superposition = os.path.join(DRAW_SRC_PATH3,f'{subject}_confronto_post.png')
...
if __name__ == '__main__':
    ...
    for fname in sorted(os.listdir(SRC_PATH)):
        ...
        xx_valid = xx[(mask_labels) & (mask_magnitude)]
        yy_valid = yy[(mask_labels) & (mask_magnitude)]

        valid_coordinates = np.column_stack([xx_valid,yy_valid])
        valid_values = optical_flow_field[mask_labels & mask_magnitude]
        ...
        if COMPUTE_METRICS:
            ...
            #---------------------------------- PLOT DEI RISULTATI (PAPER FORMAT) ----------------------------------#

            # 1. Creazione Figura (3.5 x 3.5 pollici, 300 DPI)
            fig, ax = plt.subplots(figsize=(3.5, 3.5), dpi=300)
            ax.imshow(subject_data['DRAW IMG'])
            
            # Parametri grafici adattati per il formato piccolo
            grid_color = 'white'
            thickness = 0.5
            linestyle = '--'
            ...
            ax.quiver(xx[::step_q,::step_q], yy[::step_q,::step_q], 
                      ...
                      angles='xy', color=q_color, alpha=0.8, scale=q_scale, 
            ...
            out_png = os.path.join(DST_PATH, f"{subject_data['ID']}_RBF_paper.png")
